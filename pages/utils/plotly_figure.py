...
def plotly_table(dataframe):
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(
            values=["<b></b>"] + ["<b>" + str(i) + "</b>" for i in dataframe.columns],
            line_color="#00ff00", 
            fill_color='#0078ff',
            align='center',
...
def close_chart(dataframe, num_period=False):
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['Open'],
                             mode='lines',
                             name='Open', line=dict(width=2, color="#ff5a8c")))
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['High'],
                             mode='lines',
                             name='High', line=dict(width=2, color="#ff0000")))
    ...
    fig.update_layout(height=500,
                      ...
                      plot_bgcolor='white',
                      paper_bgcolor="#ebffe1",
                      legend=dict(yanchor="top", xanchor="right"))
    return fig


def candlestick(data, title='Candlestick Chart'):
...
def Moving_average(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines',
        name='Open',
        line=dict(width=2, color="#a4ff5a")
    ))
    fig.add_trace(go.Scatter(
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines',
        name='High',
        line=dict(width=2, color="#6fff00")
    ))
    fig.add_trace(go.Scatter(
...
def MACD(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(
        x=dataframe['Date'],
        y=macd, name = 'RSI', marker_color='orange', 
        line = dict(width=2, color='orange'),
    ))
    fig.add_trace(go.Scatter(
        x=dataframe['Date'],
        y=macd_signal, name = 'Overbought', marker_color='red',
        line = dict(width=2, color='red', dash='dash'),
    ))
